...
import { BookOpen, Calendar, Clock, User, MapPin } from 'lucide-react';
import EventTypeBadge from './EventTypeBadge';

const STATUS_OPTIONS = [
  { value: 'not_started', label: 'לא התחלתי', emoji: '⚪', color: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200' },
  { value: 'in_progress', label: 'בלמידה',   emoji: '🟡', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' },
  { value: 'ready',       label: 'מוכן',     emoji: '🟢', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' },
      ...
      <DialogContent dir="rtl" className="max-w-md text-right">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
            ...
            {exam.time && <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" />{exam.time}</span>}
          ...
          {isStudentView && (
            <div className="space-y-3 pt-2 border-t">
              <div>
                <Label className="text-xs">סטטוס הכנה אישי</Label>
