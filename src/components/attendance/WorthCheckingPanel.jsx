...
export default function WorthCheckingPanel({ students, onSelectStudent, readOnly = false }) {
      ...
      <div className="p-3">
        ...
        <div className="space-y-1.5">
          {students.slice(0, 5).map(s => (
            <button key={s.id} type="button" onClick={() => onSelectStudent?.(s)} disabled={readOnly}
              className="w-full flex items-center gap-2 p-2 rounded-lg bg-white/60 dark:bg-amber-900/20 hover:bg-white dark:hover:bg-amber-900/30 transition-colors text-right disabled:cursor-default disabled:hover:bg-white/60 dark:disabled:hover:bg-amber-900/20" dir="rtl">
              ...
              {!readOnly && <ChevronLeft className="w-4 h-4 text-muted-foreground flex-shrink-0" />}
            </button>
          ))}
        </div>
      </div>
