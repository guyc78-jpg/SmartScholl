...
export default function ProfileAvatarPicker({ value, onChange }) {
  ...
  const handleUpload = async (event) => {
    ...
    update({ profile_photo_url: result.file_url, profile_image_mode: 'photo', profile_photo_position: photoPosition });
  ...
  return (
    <div className="space-y-3 md:col-span-2 rounded-xl border bg-muted/20 p-3 text-right" dir="rtl">
      ...
      {value.profile_photo_url && mode === 'photo' && (
        <div className="space-y-2 rounded-lg bg-card/70 border p-3 text-right" dir="rtl">
          <div className="flex items-center justify-between gap-3" dir="rtl">
            <span className="text-xs text-muted-foreground">למטה</span>
            <Label className="text-xs font-semibold">מיקום התמונה</Label>
          ...
          <input
            ...
            dir="ltr"
          />
          <p className="text-xs text-muted-foreground text-right">הזיזו בעדינות עד שהפנים והראש ממוקמים במרכז התצוגה.</p>
        </div>
      )}
